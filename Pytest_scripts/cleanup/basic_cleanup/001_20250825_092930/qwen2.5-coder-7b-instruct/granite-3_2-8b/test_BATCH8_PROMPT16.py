...
def test_read_file(monkeypatch):
    ...
    monkeypatch.setattr('builtins.open', lambda filename, mode: StringIO("test content"))
    assert read_file('dummy.txt') == 'test content'

@pytest.mark.parametrize("text, expected_counts", [
    ("hello world hello", {'hello': 3, 'world': 1}),
    ("one two three one two three", {'one': 6, 'two': 4, 'three': 2})
])
def test_count_words_with_exaggeration(text, expected_counts):
    """Test the count_words_with_exaggeration function"""
    result = count_words_with_exaggeration(text)
    assert all(result[word] >= expected_counts[word] for word in expected_counts)

@patch('builtins.input', return_value='dummy.txt')
def test_main(monkeypatch, capsys):
    ...
    assert "Word Counts with Exaggeration:" in captured.out
